...
| Revision | `loop-00134-8dw` (reported) — **verify memory after next deploy** |
| SHA on main (pre-ship) | `1add371` (#33 product film) |
| Memory / scale | **Should be 4Gi** · min 1 · max 2 — revision `00134` was accidentally 2Gi → GFE 429 / OOM risk |
...
**Deploy guard (this branch):** `deploy-gcp.sh` fails if the live Cloud Run revision is not **4Gi** after deploy.

**Demo tenant (Cove):** https://github.com/saurabh4269/cove · Cloud Run `cove`. Demo only — never merge Cove PRs from here.

## What works (code on main + this PR)
...
- Mail-first outreach ladder: contact lookup + cohort cluster + Gmail glass-box cards (self-inbox OAuth OK for demo).
- Customer Voice: OTP hang metrics classify `otp_verify_timeout` (not `payment_timeout`) on new ingest and telephony finalize.
- Duplicate HIGH hidden in room bundle, `/api/approvals`, and status counts when a sibling flags PR already shipped.
...
| Path | Type A HIGH |
| Voice | **Stale GCS row may still show `payment_timeout`** — new ingest / re-run uses `otp_verify_timeout` |
...
| `code_fix` | Lean worker skips; `github_pr` is the ship path |
| Verify | Often `verified=0` / inconclusive until tenant metric recovers — UI should say so honestly |

**Fresh demo room:** ingest a **unique metric** (e.g. `otp_verify_hang_0905`) so specialists run clean; same metric joins an open AWAITING_APPROVAL room when appropriate.

## Branches / PRs

### product-os

- `main` tip `1add371` (#33 merged). Next ship: demo-readiness PR (this work).
...
./scripts/deploy-gcp.sh   # exits non-zero if memory != 4Gi
```
