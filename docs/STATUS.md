...
`us-central1` Cloud Run service `loop` (bundle path, latest known revision `loop-00019-xkp`). Public `python:3.12-slim` + GCS tarball. SQLite is ephemeral (re-seeds the **full world** on cold start).
