...
# Wire live GEAP Agent Engine (loop-incident-orchestrator). Opt out: LOOP_GEAP_ENABLE=0
LOOP_GEAP_ENGINE="${LOOP_GEAP_AGENT_ENGINE_ID:-projects/mystical-timing-442601-q8/locations/us-central1/reasoningEngines/7709236223511887872}"
LOOP_GEAP_STAGING="${LOOP_GEAP_STAGING_BUCKET:-gs://${BUCKET}/agent_engine/}"
if [[ "${LOOP_GEAP_ENABLE:-1}" != "0" ]]; then
  ENV_VARS="${ENV_VARS},LOOP_GEAP_ENABLED=1,LOOP_GEAP_AGENT_ENGINE_ID=${LOOP_GEAP_ENGINE},LOOP_GEAP_STAGING_BUCKET=${LOOP_GEAP_STAGING},LOOP_GEAP_DISPLAY_NAME=loop-incident-orchestrator"
  echo "deploy-gcp: GEAP Agent Engine ${LOOP_GEAP_ENGINE}"
fi
...
if [[ -n "${LOOP_ADMIN_TOKEN:-}" ]]; then
  ENV_VARS="${ENV_VARS},LOOP_EVAL=0,LOOP_VERIFY_DEFER=1,LOOP_VERIFY_DELAY_HOURS=0,LOOP_WORKER_SECRET=${LOOP_ADMIN_TOKEN},LOOP_INLINE_WORKER=0,LOOP_AUTO_INVESTIGATE=0"
fi
